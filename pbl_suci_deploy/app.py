...
st.markdown("""
    ...
    /* ================= FORCE LIGHT THEME (ANTI BLACKOUT) ================= */
    /* Menargetkan variabel internal Streamlit agar dipaksa menjadi Light Mode */
    :root {
    ...
    /* Menyembunyikan elemen default Streamlit agar terlihat professional */
    ...
    /* JUDUL RESPONSIF: Otomatis menyesuaikan ukuran layar (Mobile Friendly) */
    .main-title {
    ...
    /* ================= FORCE INPUT FIELD TETAP PUTIH & TEKS GELAP ================= */
    ...
    /* Memaksa teks input dan background tombol penambah/pengurang (+/-) tetap putih */
    div[data-baseweb="input"] input, div[data-baseweb="input"] button {
        color: #0F172A !important;
        background-color: #FFFFFF !important;
    }
    ...
    /* ================= FORCE TABEL AGAR TETAP PUTIH BERSIH ================= */
    ...
    /* Menimpa background cell & header glide-data-grid bawaan Streamlit */
...
if hitung:
    try:
        ...
        # Grid Tabel Hasil dengan Judul Responsif
        ...
        with col_res2:
            ...
            df_input_summary = pd.DataFrame({
                'Parameter': ['N-P-K', 'Suhu/Kelembapan', 'pH Tanah', 'Curah Hujan'],
                'Nilai Input': [f"{n_input}-{p_input}-{k_input}", f"{temp_input}°C / {hum_input}%", f"{ph_input}", f"{rain_input} mm"]
            })
            st.dataframe(df_input_summary, use_container_width=True, hide_index=True)
